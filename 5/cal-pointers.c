...
int day_in_month(int is_leap_year, int month) {
  return *(*(daytab + is_leap_year) + month);  // this makes me uncomfortable
}

// set day of year from month & day
int day_of_year(int year, int month, int day) {
  int leap = is_leap_year(year);
  if (month < 0 || month > 12 || day < 0 || day > day_in_month(leap, month)) {
    return -1;
  }
